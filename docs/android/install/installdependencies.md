---
title: "Install .NET Android Dependencies"
description: "Learn how to install .NET Android Dependencies so you can create native android applications."
ms.date: 11/01/2023
---
# Installing .NET Android Dependencies

In order to build .NET Android applications you need to install two additional components,
The Java JDK and the Android SDK.

## Using "InstallAndroidDependencies"

The easiest way to install the required dependencies for your android application is to run
the `InstallAndroidDependencies` MSbuild target.
...
API 32 installed. Running the `InstallAndroidDependencies` will install API 34 for you.

If you do not have the Android SDK installed at all, this target can also handle installing the SDK
on a clean machine. You can change the destination of the installation by providing the `AndroidSDKDirectory` argument. It will also install the Java SDK if the `JavaSdkDirectory` argument is provided.
...
## Installing Android SDK Manually
...
You can use the sdkmanager to install additional components. You can use the `--list` argument to get a list of all the available components. You can then look through the list and find the additional components you want.
...
1. "platforms;android-XX" . Installs the platform `android-XX` into the sdk. Replace XX with the API Level of your chosen platform. For example "platforms;android-30" will install Android API 30, "platforms;android-21" will install Android API 21.
2. "system-images;android-XX;default;x86_64". This will install an emulator image for the specific API level. The x86_64 can be swapped out for different abis such as x86, arm64-v8a and x86_64. These reflect the ABI of the image being installed. This can be useful if you have issues on specific ABI's.
...
In order to build .NET Android applications or libraries you need to have a version of the Java Development Kit installed.
We recommend you use the Microsoft Open JDK, this has been tested against our .NET Android builds.
